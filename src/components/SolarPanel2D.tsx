...
export const SolarPanel2D = ({ azimuth, tilt }: SolarPanel2DProps) => {
  ...
  const azimuthRotation = azimuth - 90; // Offset so 0° points up (North)
  
  // Calculate panel position based on tilt
  ...
  return (
    <div className="w-full h-[400px] bg-gradient-to-b from-sky-100 to-sky-50 dark:from-slate-900 dark:to-slate-800 rounded-lg overflow-hidden border border-border p-8">
      <div className="flex h-full gap-8 items-center justify-center">
        {/* Compass View - Shows Azimuth */}
        <div className="flex-1 flex flex-col items-center justify-center gap-4">
          <h3 className="text-sm font-semibold text-foreground">Direction (Azimuth)</h3>
          <div className="relative w-48 h-48">
            ...
            <svg viewBox="0 0 200 200" className="w-full h-full">
              ...
              {/* Solar panel direction indicator */}
        ...
        {/* Side View - Shows Tilt */}
        <div className="flex-1 flex flex-col items-center justify-center gap-4">
          <h3 className="text-sm font-semibold text-foreground">Tilt Angle</h3>
            ...
            <svg viewBox="0 0 200 150" className="w-full h-full">
              ...
              <g transform={`rotate(${-tilt} 100 ${120 - panelHeight})`}>
                ...
                {[0.25, 0.5, 0.75].map((fraction) => (
                  <line
                    key={fraction}
                    x1={100 - panelWidth / 2 + panelWidth * fraction}
                    y1={120 - panelHeight - 5}
                    x2={100 - panelWidth / 2 + panelWidth * fraction}
                    y2={120 - panelHeight + 5}
                    stroke="hsl(var(--primary-foreground))"
                    strokeWidth="1"
                    opacity="0.5"
                  />
              ...
              {/* Angle arc */}
              <path
                d={`M ${100 + 30} 120 A 30 30 0 0 1 ${100 + 30 * Math.cos(tiltRadians)} ${120 - 30 * Math.sin(tiltRadians)}`}
                fill="none"
